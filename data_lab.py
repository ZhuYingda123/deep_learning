...
path1 = '/data/dataDisk2/yingda/return'
...
def estab_all_tag_data(day_list):
    tag_all = ['totoff','vol','totbid','amount','last','low','high','open','avebid','aveoff','trade','bid1','ask1','bid_vol1','ask_vol1']
    tag_accum = ['amount','trade','vol','totbid','totoff']
    tag_dtype_int64 = ['totoff','vol','totbid','amount']
    # all_tag_data = []
    for i, day in enumerate(day_list):
        all_tag_array = np.zeros((3627, len(tag_all)*4801))
        '''
        check whether tick data exists 
        '''
        year = str(day // 10000).zfill(4)
        ...
        for i2, tag in enumerate(tag_all):
            ...
            if re.search(r'ask[0-9]+',tag)!=None or re.search(r'bid[0-9]+',tag)!=None:
                ...
                array_data = array_data[:, 1:]
            fullfill_data(array_data)
            all_tag_array[:, np.arange(4801)*len(tag_all)+np.ones(4801, dtype='int64')*i2]=array_data[:, np.arange(4801)].copy()
            if i==0:
...
if __name__ == '__main__':
    ...
    day_list = estab_day_list(20190413, 20190430)
    ...
    label_data = get_lab(path=path1, start='20190401',end='20190412').values
